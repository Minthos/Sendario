...
void main() {
    ...
    //float inclination = velocity.x / pi;
    //float insolation = velocity.y + 0.5;
    ...
    if(elevation < 1.0){
        ...
        fragColor = vec4(velocity.x, velocity.y, sum(color * (3.0 + insolation)) / 12.0, 1.0);
    } else if(elevation < 100.0) {
        //fragColor = vec4(sand * insolation, 1.0);
        fragColor = vec4(velocity.x, velocity.y, sum(sand * insolation) / 3.0, 1.0);
    } else {
        //fragColor = vec4(color * insolation, 1.0);
        fragColor = vec4(velocity.x, velocity.y, sum(color * insolation) / 3.0, 1.0);
    }
    velocityOut = vec2(100, 100); // not getting through from here to our pp_motionblurfrag shader apparently
//    velocityOut = velocity; // not getting through from here to our pp_motionblurfrag shader apparently
